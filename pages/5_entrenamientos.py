...
# ==============================================================================
#  CARGA DIRECTA (UNO A UNO)
# ==============================================================================
with tab_cargar:
    with st.container(key=f"flow_container_{st.session_state.form_iteration}"):
        ...
        if rol == "N":
            n_in = col_nad = c2.selectbox("Nadador", [mi_nom_comp], disabled=True)
            id_nad_final = mi_id
        else:
        ...
        if n_in and est_val and dist_t_val:
            ...
            # Validación duplicados en tiempo real contra la base
            ...
            if existe:
                st.error(f"🚫 Ya existe un registro para {n_in} con esta configuración en la fecha seleccionada.")
            else:
                # Regla de parciales
                ...
                with st.form("f_registro_directo"):
                    ...
                    if quiere_parciales:
                        ...
                        for i in range(1, 5):
                            ...
                            parciales_inputs.append((pm, ps, pc))
                    ...
                    if st.form_submit_button("GUARDAR REGISTRO", use_container_width=True):
                        ...
                        if seg_final == 0:
                            ...
                            suma_p = sum([tiempo_a_segundos(p[0], p[1], p[2]) for p in parciales_inputs]) if quiere_parciales else 0
                            if quiere_parciales and suma_p > 0 and abs(suma_p - seg_final) > 0.5:
                                st.warning(f"⚠️ La suma de parciales ({suma_p:.2f}s) no coincide con el final ({seg_final:.2f}s).")
                            ...
                            with st.spinner("Guardando en la nube..."):
                                try:
                                    ...
                                    nuevo_df = pd.DataFrame([{
                                        ...
                                        "parcial_1": tiempo_str(*parciales_inputs[0]) if len(parciales_inputs)>0 and sum(parciales_inputs[0])>0 else "",
                                        "parcial_2": tiempo_str(*parciales_inputs[1]) if len(parciales_inputs)>1 and sum(parciales_inputs[1])>0 else "",
                                        "parcial_3": tiempo_str(*parciales_inputs[2]) if len(parciales_inputs)>2 and sum(parciales_inputs[2])>0 else "",
                                        "parcial_4": tiempo_str(*parciales_inputs[3]) if len(parciales_inputs)>3 and sum(parciales_inputs[3])>0 else "",
                                        "observaciones": obs
                                    }])
                                    
                                    df_actualizado = pd.concat([df_ent, nuevo_df], ignore_index=True)
                                    conn.update(worksheet="Entrenamientos", data=df_actualizado)
                                    
                                    st.cache_data.clear()
                                    st.success("✅ Registro guardado con éxito.")
                                    # Reinicio total del flujo
                                    forzar_reinicio_total()
                                    st.rerun()
...
with tab_ver:
    ...
    if rol in ["M", "P"]:
        s_n = st.selectbox("Historial de:", lista_nombres, index=None, key="hist_sel")
    ...
    if t_id:
        ...
        else: st.info("No hay registros para este nadador.")
