...
# --- CSS PERSONALIZADO (Restaurado completo) ---
st.markdown("""
<style>
...
with tab_cargar:
    ...
    if n_in and est_val and dist_t_val:
        ...
        # VALIDACIÓN DE DUPLICADOS (Robusta contra tablas vacías)
        ...
        if existe_db or existe_cola:
            st.error(f"🚫 Error: Ya existe un registro para {dist_t_val} en {est_val} este día.")
        else:
            # Regla Automática
...
if st.session_state.cola_tests:
    st.divider()
    st.info(f"📋 {len(st.session_state.cola_tests)} tests listos para subir.")
