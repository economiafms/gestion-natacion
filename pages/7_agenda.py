        ...
        with st.container():
            ...
            with st.expander("📋 Ver Lista de Inscriptos"):
                if d_full.empty:
                    ...
                    filtro_cat = c_f1.selectbox("Categoría", lista_cats)
                    
                    # Filtro Género
                    lista_gen = ["Todos"] + sorted(d_full['codgenero'].unique().tolist())
                    filtro_gen = c_f2.selectbox("Género", lista_gen)
                    
                    # Filtro Prueba
                    lista_pruebas_inscriptas = []
                    for p_list in d_full['pruebas'].str.split(','):
                        for p in p_list:
                            if p.strip() not in lista_pruebas_inscriptas:
                                lista_pruebas_inscriptas.append(p.strip())
                    
                    filtro_prueba = c_f3.selectbox("Prueba", ["Todas"] + sorted(lista_pruebas_inscriptas))
