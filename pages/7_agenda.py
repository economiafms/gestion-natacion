...
if df_competencias is None or df_competencias.empty:
    ...
    for _, row in df_view.iterrows():
        ...
        with st.container():
            ...
            # === A. LISTA PÚBLICA DE INSCRIPTOS (CON FILTROS Y TARJETAS VISUALES) ===
            with st.expander("📋 Ver Lista de Inscriptos"):
                if d_full.empty:
                    ...
                    # --- 3. RENDERIZADO VISUAL (TARJETAS + TIEMPOS) ---
                    if df_filtrado.empty:
                        st.info("Ningún nadador coincide con los filtros seleccionados.")
                    else:
                        st.markdown(f"<div style='margin-bottom: 15px; color: #aaa; font-size: 14px;'>Mostrando {len(df_filtrado)} inscriptos:</div>", unsafe_allow_html=True)
                ...
                with st.expander(f"🛡️ Panel Entrenador ({row['nombre_evento']})"):
                    ...
                    with t2:
                        with st.form(f"ed_{comp_id}"):
                            ...
                            nf = st.date_input("Fecha", value=pd.to_datetime(row['fecha_dt']), format="DD/MM/YYYY")
                            nl = st.date_input("Cierre", value=pd.to_datetime(f_lim), format="DD/MM/YYYY")
                            ...
                            if st.form_submit_button("Actualizar"): 
                                guardar_competencia(comp_id, nn, nf, row['hora_inicio'], row['cod_pileta'], nl, nc, nd, nh); st.rerun()
