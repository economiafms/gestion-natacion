...
if df_competencias is None or df_competencias.empty:
    ...
    for _, row in df_view.iterrows():
        ...
        with st.container():
            ...
            # === A. LISTA PÚBLICA DE INSCRIPTOS (CON FILTRO AÑADIDO) ===
            with st.expander("📋 Ver Lista de Inscriptos"):
                if d_full.empty:
                    st.caption("Aún no hay nadadores inscriptos.")
                else:
                    # Determinamos las pruebas que están habilitadas en este evento
                    p_hab_str = str(row.get('pruebas_habilitadas', ""))
                    p_hab = [x.strip() for x in p_hab_str.split(",")] if p_hab_str.strip() else LISTA_PRUEBAS
                    
                    # El selector que pediste (Filtro por prueba)
                    filtro_prueba = st.selectbox(
                        "🔍 Filtrar inscriptos por prueba:", 
                        options=["Todas"] + p_hab, 
                        key=f"filtro_pub_{comp_id}"
                    )
                    
                    # Aplicamos el filtro al dataframe antes de mostrarlo
                    df_mostrar = d_full.copy()
                    if filtro_prueba != "Todas":
                        # Filtramos donde la columna "pruebas" contiene la prueba seleccionada
                        df_mostrar = df_mostrar[df_mostrar['pruebas'].str.contains(filtro_prueba, case=False, na=False)]
                    
                    if df_mostrar.empty:
                        st.info(f"Nadie se anotó a '{filtro_prueba}' todavía.")
                    else:
                        st.dataframe(
                            df_mostrar[['Nombre', 'codgenero', 'Cat', 'pruebas']].rename(columns={'codgenero':'Gen', 'pruebas':'Pruebas Inscriptas'}),
                            hide_index=True,
