...
def cerrar_sesion():
    ...
    # Limpiamos la selección al salir
    if "selected_nadador_m" in st.session_state: del st.session_state.selected_nadador_m
    st.rerun()
...
def render_personal_card(user_id, db):
    ...
    # --- BOTÓN CORREGIDO PARA SETEAR MEMORIA ---
    if st.button("Ver Mi Ficha Completa ➝", key="btn_ficha_main", type="primary", use_container_width=True):
        st.session_state.selected_nadador_m = st.session_state.user_name # Forzamos la variable del widget
