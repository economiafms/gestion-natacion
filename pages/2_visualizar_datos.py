...
with tab1:
    ...
    df_p['Edad'] = 2025 - df_p['fechanac'].dt.year
...
with tab2:
    ...
    if f_nad:
        id_actual = df_nad[df_nad['Nombre Completo'] == f_nad]['codnadador'].values[0]
        st.header(f"📊 Ficha Atleta: {f_nad}")
        ...
        c_m1.metric("🥇 Oros", int(oro)); c_m2.metric("🥈 Platas", int(plata)); c_m3.metric("🥉 Bronces", int(bronce)); c_m4.metric("📊 Total Podios", int(oro+plata+bronce))
        ...
        # --- HISTORIAL INDIVIDUAL (FILTROS COMUNICADOS) ---
        ...
        # --- MIS RELEVOS (FILTROS COMUNICADOS Y MARCAS FILTRADAS) ---
        ...
        if not mis_r.empty:
            ...
            # Comunicación Estilo -> Género/Distancia
            ...
            # Filtro 3: Distancia (Dependiente de Estilo y Género)
            fr_dis_sel = cr3.selectbox("Distancia Relevo:", ["Todos"] + sorted(df_rel_temp['Distancia'].unique().tolist()), key="fr_rel_dis")

            if fr_dis_sel != "Todos":
                df_rel_temp = df_rel_temp[df_rel_temp['Distancia'] == fr_dis_sel]
            ...
            st.dataframe(df_rel_temp[['fecha', 'Estilo', 'Distancia', 'codgenero', 'tiempo_final', 'posicion', 'Sede_Full', 'Equipo y Marcas']].sort_values('fecha', ascending=False),
                         use_container_width=True, hide_index=True)

# --- TAB 3: TODOS LOS RELEVOS (FILTROS COMUNICADOS) ---
with tab3:
    ...
    if not data['relevos'].empty:
        ...
        # Filtro 3: Reglamento (Dependiente)
        ...
        # Procesamiento de categorías para la tabla final
        ...
        for i in range(1, 5):
            ...
            mr_temp['suma_edades'] += (2025 - nacs.dt.year).fillna(0)
