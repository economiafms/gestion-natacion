...
def cargar_datos_sim():
    try:
        ...
        # Filtrar mejores tiempos en 50m
...
def analizar_competitividad(tiempo_seg, suma_edades, genero):
    ...
    if cat_techo in benchmarks.get(genero, {}):
        ...
        if tiempo_seg <= meta: return f"🔥 **NIVEL PODIO.** Tiempo de referencia: {seg_a_tiempo(meta)}."
        elif tiempo_seg <= meta + 10: return f"✨ **COMPETITIVO.** Cerca de marcas de podio."
    return ""
...
def render_tarjeta_resumen(tiempo, categoria, suma, dark=False):
    ...
    st.markdown(f"""
        <div style='background-color: {bg}; padding: 12px; border-radius: 10px; border-left: 8px solid #E30613; color: {text}; margin-bottom: 15px;'>
            <div style='display: flex; justify-content: space-around; align-items: center; flex-wrap: wrap;'>
                ...
                <div style='text-align: center;'>
                    <b style='font-size: 18px; color: #E30613;'>{categoria.upper()}</b><br>
...
# --- 6. INTERFAZ: SIMULADOR POR GRUPO ---
st.divider()
...
if st.button("🪄 Generar Estrategia", type="primary", use_container_width=True):
    if len(pool) < 4: st.warning("Seleccione al menos 4 nadadores.")
        ...
        with st.spinner("Procesando combinaciones..."):
            ...
            if not resultados: st.info("No hay combinaciones válidas para los criterios seleccionados.")
            else:
