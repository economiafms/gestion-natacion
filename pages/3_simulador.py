...
st.markdown("<h4 style='text-align: center;'>Optimización Newell's Old Boys - Rumbo a la Copa</h4>", unsafe_allow_html=True)
...
def get_cat_info(suma, reg):
    ...
    for _, r in regs.iterrows():
        if r['suma_min'] <= suma <= r['suma_max']: 
            return r['descripcion'], r['suma_max']
    return f"Suma {int(suma)}", 999

def analizar_competitividad(tiempo_seg, suma_edades, genero):
    # Benchmarks ajustados con segundos extra para ser "razonables" (Nivel Regional Master)
    # Valores en segundos totales para 4x50m
    benchmarks = {
        "M": {119: 122, 159: 125, 199: 132, 239: 145}, # Varones (Meta < 180 ~ 2:05)
        "F": {119: 140, 159: 150, 199: 160, 239: 180}, # Mujeres (Meta < 180 ~ 2:30)
        "X": {119: 128, 159: 135, 199: 140, 239: 160}  # Mixtos
    }
    ...
    if cat_techo in benchmarks[genero]:
        ...
        if tiempo_seg <= meta:
            return f"🔥 **TIEMPO COMPETITIVO.** Están por debajo de la marca de referencia de {seg_a_tiempo(meta)}."
        elif tiempo_seg <= meta + 8:
            return f"✨ **CERCA DEL PODIO.** A solo {seg_a_tiempo(tiempo_seg - meta)} del tiempo objetivo."
...
with st.container(border=True):
    ...
    if st.button("🚀 Calcular Simulación Real", use_container_width=True):
        if len(set(n_sel)) == 4 and None not in n_sel:
            ...
            st.success(f"**Categoría: {cat_desc} | Tiempo Total: {seg_a_tiempo(total)}**")
            st.info(analizar_competitividad(total, se, s_gen))
        else:
...
st.caption("Arma las mejores postas posibles sin repetir nadadores y analiza el salto de categoría.")

pool = st.multiselect("Nadadores convocados (Pool de atletas):", sorted(df_nad['Nombre Completo'].tolist()))
...
if st.button("🪄 Generar Estrategia de Podios", type="primary", use_container_width=True):
    if len(pool) < 4:
        ...
        while len(pool_actual) >= 4:
            ...
            propuestas.append({
                'equipo': mejor_eq['equipo'], 'tiempo': mejor_eq['tiempo'], 
                'cat': cat_nom, 'suma': se_e, 'suma_max': cat_max,
        ...
        if propuestas:
            st.success(f"✅ Se formaron {len(propuestas)} equipos estratégicos.")
            for i, p in enumerate(propuestas):
                with st.expander(f"Posta #{i+1}: {p['cat']} (Proyección: {seg_a_tiempo(p['tiempo'])})", expanded=True):
                    ...
                    for j in range(4):
                        ...
                        cols_p[j].markdown(f"<small>{p['equipo'][j]}</small>", unsafe_allow_html=True)
                        cols_p[j].code(seg_a_tiempo(p['parciales'][j]))
                    
                    # Análisis de Competitividad
                    st.write(analizar_competitividad(p['tiempo'], p['suma'], o_gen))
                    
                    # Recomendación de Salto de Categoría
                    faltante = p['suma_max'] - p['suma']
                    if faltante <= 8:
                        st.info(f"💡 **TIP ESTRATÉGICO:** Este equipo suma {p['suma']} años. Si reemplazan a un nadador por uno {faltante + 1} años mayor, saltan a la siguiente categoría. A veces conviene 'perder' unos segundos pero competir contra nadadores de más edad.")
        else:
            st.error("No se pudieron formar equipos válidos. Verifica las marcas de los nadadores seleccionados.")
