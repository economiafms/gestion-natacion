...
def analizar_competitividad(tiempo_seg, suma_edades, genero):
    ...
    if cat_techo in benchmarks.get(genero, {}):
        ...
        elif tiempo_seg <= meta + 10: return f"✨ **NIVEL COMPETITIVO.** Cerca de tiempos de podio nacional."
    return ""
...
with st.container(border=True):
    ...
    n_sel = []
    cols = st.columns(2) # En celular, 2 columnas es más cómodo
    ...
    if st.button("🚀 Calcular Posta", use_container_width=True):
        if len(set(n_sel)) == 4 and None not in n_sel:
            ...
            if s_gen == "X" and (gens.count("M") != 2 or gens.count("F") != 2):
                ...
                # --- TARJETA COMPACTA ---
                st.markdown(f"""
                <div style='background-color: #f8f9fa; padding: 10px; border-radius: 8px; border-left: 5px solid red; color: #333; margin-bottom: 15px;'>
                ...
                if not hist.empty:
                    ...
                    obs += f"\n\n📋 **ANT:** {ant['tiempo_final']} en **{ip['club']} ({ip['medida']})** el {ant['fecha']}."
                ...
                if mejor_t < total:
                    obs += f"\n\n💡 **ORDEN:** Mejor tiempo {seg_a_tiempo(mejor_t)} alternando orden."
...
if st.button("🪄 Generar Estrategia Óptima", type="primary", use_container_width=True):
    if len(pool) < 4: st.warning("⚠️ Seleccione al menos 4 nadadores.")
        ...
        with st.spinner("Analizando..."):
            ...
            if not propuestas:
                st.info("⚠️ **OBSERVACIÓN:** No se pudieron conformar relevos con los datos y género solicitados.")
            else:
                for i, p in enumerate(propuestas):
                    with st.expander(f"POSTA #{i+1}: {p['cat'].upper()} ({seg_a_tiempo(p['t'])})", expanded=True):
                        # --- TARJETA CLARA EN SIMULADOR ---
                        st.markdown(f"""
                        <div style='background-color: #1e1e1e; padding: 10px; border-radius: 5px; color: white; display: flex; justify-content: space-around; font-size: 14px;'>
                            <span><b>TIEMPO:</b> {seg_a_tiempo(p['t'])}</span>
                            <span><b>CATEGORÍA:</b> {p['cat'].upper()}</span>
                            <span><b>SUMA:</b> {p['se']} años</span>
                        </div>
                        """, unsafe_allow_html=True)
                        cs = st.columns(4)
                        for j in range(4):
