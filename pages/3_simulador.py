...
# Usamos "centered" para que en desktop se vea contenido y en mobile ocupe todo el ancho natural
st.set_page_config(page_title="Simulador Pro", layout="centered")

# --- 2. SEGURIDAD ---
if "role" not in st.session_state or not st.session_state.role:
    st.switch_page("index.py")

# CSS para ajustar márgenes en móvil y evitar saltos bruscos
st.markdown("""
    <style>
        .block-container { padding-top: 2rem; padding-bottom: 5rem; }
        h3 { margin-bottom: 0; }
        /* Ajuste de métricas para que no se rompan en mobile */
        [data-testid="stMetricValue"] { font-size: 1.2rem !important; }
        [data-testid="stMetricLabel"] { font-size: 0.8rem !important; }
    </style>
""", unsafe_allow_html=True)

st.markdown("<h3 style='text-align: center; color: #E30613;'>🔴⚫ SIMULADOR NOB</h3>", unsafe_allow_html=True)
...
def analizar_competitividad(tiempo_seg, suma_edades, genero):
    ...
    if cat_techo in benchmarks.get(genero, {}):
        ...
        if tiempo_seg <= meta: return f"🔥 **PODIO.** Ref: {seg_a_tiempo(meta)}"
...
def render_tarjeta_resumen(tiempo, categoria, suma, dark=False):
    ...
    # Diseño más compacto para mobile
    st.markdown(f"""
        <div style='background-color: {bg}; padding: 10px; border-radius: 8px; border-left: 6px solid #E30613; color: {text}; margin-bottom: 10px;'>
            <div style='display: flex; justify-content: space-between; align-items: center;'>
                <div style='text-align: left;'>
                    <div style='font-size: 11px; color: #888; text-transform: uppercase;'>Tiempo Final</div>
                    <div style='font-size: 24px; font-family: monospace; font-weight: bold;'>{tiempo}</div>
                </div>
                <div style='text-align: right;'>
                    <div style='font-size: 14px; color: #E30613; font-weight: bold;'>{categoria}</div>
                    <div style='font-size: 12px; color: #888;'>Suma: {suma} años</div>
                </div>
            </div>
        </div>
    """, unsafe_allow_html=True)
...
# --- 5. POSTA MANUAL ---
st.write("") 
with st.expander("🧪 **Armar Posta Manual**", expanded=True):
    c1, c2 = st.columns(2)
    s_reg_m = c1.selectbox("Reglamento", data['cat_relevos']['tipo_reglamento'].unique(), key="s_reg_m")
    s_gen_sel = c2.selectbox("Género", ["Masculino (M)", "Femenino (F)", "Mixto"], key="s_gen_m")
    s_tipo_m = st.radio("Prueba", ["Libre (Crol)", "Combinado (Medley)"], horizontal=True, key="s_tipo_m")
    ...
    # Selectores Apilados para Mobile (más fácil de tocar)
...
# --- ZONA DE RESULTADOS (PLACEHOLDER) ---
# Esto evita el salto. El contenedor está reservado.
res_manual = st.container()

if btn_calc:
    if None in n_sel:
        st.warning("⚠️ Faltan nadadores.")
    elif len(set(n_sel)) < 4:
        st.error("⛔ Nadador repetido.")
    else:
        with st.spinner("Calculando..."):
            ...
            with res_manual:
                ...
                # Grilla 2x2 para móviles
                r1, r2 = st.columns(2)
                r1.metric("1. " + legs[0][0], seg_a_tiempo(tiempos_p[0]))
                r2.metric("2. " + legs[1][0], seg_a_tiempo(tiempos_p[1]))
                
                r3, r4 = st.columns(2)
                r3.metric("3. " + legs[2][0], seg_a_tiempo(tiempos_p[2]))
                r4.metric("4. " + legs[3][0], seg_a_tiempo(tiempos_p[3]))
                ...
                if not hist.empty:
                    ...
                    obs_lista.append(f"⏱️ **ANTECEDENTE:** {ant['tiempo_final']} ({ip['club']})")
...
# --- 6. SIMULADOR GRUPO (Colapsado por defecto) ---
st.write("")
with st.expander("🎯 **Simulador Automático (Grupo)**", expanded=False):
    pool = st.multiselect("Nadadores (min 4):", sorted(df_nad['Nombre Completo'].tolist()), key="pool_opt_g")
    
    gc1, gc2 = st.columns(2)
    o_reg = gc1.selectbox("Reglamento", data['cat_relevos']['tipo_reglamento'].unique(), key="o_reg_g")
    o_gen_sel = gc2.selectbox("Género", ["Masculino (M)", "Femenino (F)", "Mixto"], key="o_gen_g")
    o_tipo = st.radio("Prueba", ["Libre (Crol)", "Combinado (Medley)"], horizontal=True, key="o_tipo_g")
...
# Placeholder para resultados automáticos
res_auto = st.container()

if btn_auto:
    if len(pool) < 4:
        st.warning("Selecciona más nadadores.")
    else:
        with st.spinner("Procesando..."):
            ...
            with res_auto:
                st.divider()
                if not resultados: st.info("Sin combinaciones válidas.")
                else:
