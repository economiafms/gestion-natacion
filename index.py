...
if "role" not in st.session_state or not st.session_state.role:
    st.switch_page("index.py") # Si entran directo sin loguearse, fuera.
...
# --- 5. COMPONENTES VISUALES (El diseño clásico) ---

def render_personal_card(user_id):
    if not db: return
...
if db and st.session_state.user_id:
    # 1. TARJETA PERSONAL
    render_personal_card(st.session_state.user_id)

    # 2. ESTADÍSTICAS DEL CLUB (Diseño Original)
    ...
    # 3. GRÁFICOS
    render_graficos_comunes()

# --- 4. ZONA DE HERRAMIENTAS (Solo para Master/Profe) ---
if st.session_state.role in ["M", "P"]:
    st.divider()
    
    # Botones grandes de navegación directa
    c1, c2 = st.columns(2)
    with c1: 
    ...
    # --- ZONA DE ACCESO PROFESOR (Candado) ---
    st.write(""); st.write("")
    ...
    if st.session_state.admin_unlocked:
        st.success("🔓 Gestión Habilitada (Menú lateral actualizado)")
